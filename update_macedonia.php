...
class MacedoniaUpdater {
    ...
    public function getMacedoniaInfo($id) {
        $sql = "SELECT * FROM macedonia WHERE id = ?";
    ...
    public function updateMacedonia($id, $name, $description, $image) {
        ...
        $sql = ($file)
            ? "UPDATE macedonia SET name=?, description=?, image=? WHERE id=?"
            : "UPDATE macedonia SET name=?, description=? WHERE id=?";
        $stmt = $this->conn->prepare($sql);
        if ($file) {
...
$macedoniaUpdater = new MacedoniaUpdater($conn);

if ($_GET['macedonia_id']) {
    $info = $macedoniaUpdater->getMacedoniaInfo($_GET['macedonia_id']);
}

if (isset($_POST['update_product'])) {
    $macedoniaUpdater->updateMacedonia($_POST['id'], $_POST['name'], $_POST['description'], $_FILES['image']['name']);
}
