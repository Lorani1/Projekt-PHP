...
class KosovoUpdater {
    ...
    public function getKosovoInfo($id) {
        $sql = "SELECT * FROM kosova WHERE id = ?";
    ...
    public function updateKosovo($id, $name, $description, $image) {
        ...
        $sql = ($file)
            ? "UPDATE kosova SET name=?, description=?, image=? WHERE id=?"
            : "UPDATE kosova SET name=?, description=? WHERE id=?";
        $stmt = $this->conn->prepare($sql);
        if ($file) {
...
$kosovoUpdater = new KosovoUpdater($conn);

if ($_GET['kosova_id']) {
    $info = $kosovoUpdater->getKosovoInfo($_GET['kosova_id']);
}

if (isset($_POST['update_product'])) {
    $kosovoUpdater->updateKosovo($_POST['id'], $_POST['name'], $_POST['description'], $_FILES['image']['name']);
}
